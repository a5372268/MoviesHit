                                ...
                                <div class="table-responsive">
                                   <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/showtime/showtime.do" name="form1">
                                    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0" style="text-align:left;">
                                        ...
                                        <tbody>
											<tr>
												...
												<td width="800px;">
													<select name="movie_no" style="width:189px;" required>
														<option value="">請選擇電影</option>
														<c:forEach var="movieVO" items="${movieSvc.all}" >
															<c:if test="${movieVO.offdate.compareTo(date)>(-1)}">
																<option class="${movieVO.offdate}" name = "${movieVO.length}" value= "${movieVO.movieno}">${movieVO.moviename}</option>
															</c:if>
														</c:forEach>
													</select>
 												</td>
											...
											<tr>
												...
												<td>
													<select name="theater_no" style="width:189px;" required>
														<option value= "">請選擇廳院</option>
											...
											<tr>
												<td><span style="font-weight:bolder;">開始日期</span></td>
												<td>
											...
											<tr>
												<td><span style="font-weight:bolder;">結束日期</span></td>
												<td>
													<input name="showtime_date" class="f_date1" type="text">
												</td>
												<td></td>
											...
											<tr>
												...
												<td>
													...
													class="btn btn-outline-danger" style="visibility:hidden;float:right; border:2px #B7B7B7 solid;border-radius:10px; background-color:#FF4268; font-weight:bold; color:white;">
                                    ...
                                    <table class="table table-bordered" id="table" width="100%" cellspacing="0" style="text-align:left;">
                                        <tbody>
											<tr>
												<td style="width:133px; background-color:#524066; font-size:22px; color:white">
													<span style="font-weight:bolder; ">場次編號</span>
												</td>
												<td style="width:825px; background-color:#524066; text-align:center; font-size:30px; color:white">
													<span style="font-weight:bolder; ">時間</span>
												</td>
												<td style="width:124px; background-color:#524066;"></td>
											</tr>
                                        </tbody>
                                    </table>
                                	<input type="hidden" name="action" value="insert2">
									<input type="submit" value="送出新增" id="send"
									class="btn btn-outline-danger" style="margin-top:20px;float:right; border:2px #B7B7B7 solid;border-radius:10px; background-color:#FF4268; font-weight:bold; color:white;">
...
<script>
        ...
        $('.f_date1').datetimepicker({
           ...
           //maxDate:               '+1970-01-01'  // 去除今日(不含)之後
            ...
            onChangeDateTime:function(dp, $dom){
			   if((Date.parse($dom.parent().parent().parent().find("input").eq(1).val())).valueOf() 
					   < (Date.parse($dom.parent().parent().parent().find("input").eq(0).val())).valueOf()){
				   swal.fire({
						 icon: 'error',
						 title: '錯誤...',
						 text: "開始日期在結束日期之後",
					});	
		    	   $("input[name=showtime_date]").eq(0).val($("input[name=showtime_date]").eq(1).val())
           		}
            }
           });
        ...
        //      2.以下為某一天之後的日期無法選擇
        $("select[name=movie_no]").change(function(){
        	let endDate = $("select[name=movie_no] :selected").attr("class");
				somedate2 = new Date(endDate);
        })
...
//         $("#send").click(function(){
//         	let $showtime = $("#table").find("input");
//         	for(let i = 0; i < $showtime.length; i++){
//         		console.log(i);
//         		let showtime = $showtime.eq(i).val();
//         		console.log(showtime)
//         		let $date = $("input[name=showtime_date]");
//         		let start_date = $date.eq(0).val();
//         		let end_date = $date.eq(1).val();
// 				let movie_no = $("select[name=movie_no]").val();        		
// 				let theater_no = $("select[name=theater_no]").val();        		
        		
//         		$.ajax({
<%--             		url: "<%=request.getContextPath()%>/showtime/showtime.do?", --%>
//             		type: "POST",
//             		data: {	
//             				action:"check_showtime1",
//             				movie_no: movie_no, 
