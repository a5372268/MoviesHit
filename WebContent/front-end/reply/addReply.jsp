...
<table id="table-1">
	<tr><td>
		 <h3>員工資料新增 - addReply.jsp</h3>
		 <h4><a href="<%=request.getContextPath()%>/front-end/reply/select_page.jsp"><img src="<%=request.getContextPath()%>/front-end/reply/images/tomcat.png" width="100" height="32" border="0">回首頁</a></h4>
	</td></tr>
</table>
...
<table>
	...
	<tr>
		...
		<td><input type="TEXT" name="article_no" size="45" 
			 value="<%=(replyVO==null)? "1" : replyVO.getArticle_no()%>" /></td>
	</tr>
	<tr>
		<td>會員編號:</td>
		<td><input type="TEXT" name="member_no" size="45" 
			 value="<%=(replyVO==null)? "1" : replyVO.getMember_no()%>" /></td>
	</tr>
	<tr>
		<td>回覆內容:</td>
				<td><input type="TEXT" name="content" size="45"
			 value="<%= (replyVO==null)? "確定可以嗎" : replyVO.getContent()%>" /></td>
	</tr>
	...
	<tr>
		...
		<td><input type="TEXT" name="status" size="45"
			 value="<%= (replyVO==null)? "0" : replyVO.getStatus()%>" /></td>
	</tr>
...
<%
  ...
  try {
	 	 modify_dt = replyVO.getModify_dt();
   } catch (Exception e) {
